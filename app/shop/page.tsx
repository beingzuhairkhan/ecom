...
export default function ShopPage() {
  ...
  const categories = [...new Set(FEATURED_PRODUCTS.map(p => p.category))];
  
  // Get unique vendors
  const vendors = [...new Set(FEATURED_PRODUCTS.map(p => p.vendor))];
      ...
      <div className="bg-gray-50 py-8">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between mb-6">
            <h1 className="text-3xl font-bold text-[#122B5F] mb-4 md:mb-0">Shop</h1>
                      ...
                      <div key={rating} className="flex items-center">
                        ...
                        >
                          {Array(rating).fill(0).map((_, i) => (
                            <svg key={i} className="h-4 w-4 text-[#F4A826] fill-[#F4A826]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                              <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
                            </svg>
                          ))}
                          {Array(5 - rating).fill(0).map((_, i) => (
                            <svg key={i} className="h-4 w-4 text-gray-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                              <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
                            </svg>
                          ))}
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex justify-between items-center mb-6">
                  <p className="text-gray-600">
                    Showing <span className="font-medium">{FEATURED_PRODUCTS.length}</span> products
                  </p>
